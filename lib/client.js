(function () {
  ...
  exports.create = function (config) {
    ...
    var doer = {
      ...
      request: function (route, callback, data, masterList, pathToDataArr) {
        ...
        //<https://grow.recurly.com/v2/accounts?per_page=200&state=active>; rel="start", <https://grow.recurly.com/v2/accounts?cursor=-1545493243876692566&per_page=200&state=active>; rel="prev", <https://grow.recurly.com/v2/accounts?cursor=1591224566771534796&per_page=200&state=active>; rel="next"

        //<https://grow.recurly.com/v2/accounts?cursor=1606246804508914907&per_page=200&state=active>; rel="next"

        if (method.toLowerCase() === 'post' || method.toLowerCase() === 'put') {
          options.headers['Content-Type'] = 'application/xml';
          that.debug(data);
        }
        if (_.startsWith(endpoint, '/v2/transactions')) { // In order to enable date filtering on this endpoint, API version must be at least 2.12.
          options.headers['X-Api-version'] = 2.12
        }
        that.debug(options);
        var req = https.request(options, function (res) {
          ...
          res.on('end', function () {
            ...
            var reg = /(\/v2\/.{4,14}\?cursor=[0-9]+.*?)>; rel="next"/g;
            var myStr = res.headers.link;
            var match = reg.exec(myStr);

            if (match) {
              route[0] = match[1];
              return parser.parseString(responsedata, function (err, result) {
                masterList = masterList.concat(_.get(result, pathToDataArr));
                return doer.request(route, callback, data, masterList, pathToDataArr);
              });
              ...
              try {
                ...
                if (res.statusCode >= 200 && res.statusCode <= 299) {
                  ...
                  return parser.parseString(responsedata, function (err, result) {
                    //var data = _.get(result, pathToDataArr);
                    //data = masterList.concat(data);
                    _.set(result, pathToDataArr, masterList.concat(_.get(result, pathToDataArr)));
